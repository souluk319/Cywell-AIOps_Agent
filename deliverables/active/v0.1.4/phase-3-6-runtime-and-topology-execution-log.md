...
## Latest Update - CRC Release Evidence Refresh and Console Plugin Alignment
...
- Regenerated current clean local evidence: `npm run verify:deploy:manifests` is PASS with 311 checks after durable console DOM evidence contract checks, and `$env:CAS_PBS_SOURCE_HEAD="6604777abb9e6bd44a83c6a12f36e31ac396489e"; npm run verify:release:source-pinning` is PASS with 62 checks.
- `npm run verify:pbs:preflight:live:site:preapply` is expected FAIL with 56 PASS / 4 FAIL after collapsing duplicate missing-Secret NotFound checks into the required Secret checks. The stale release evidence and per-image digest blockers are closed; remaining blockers are missing `cas-pbs-auth`, missing `cas-knowledge-postgres-live`, missing real live-prereq render/hash binding, and legacy CRC dev Secret cleanup.
- `npm run render:pbs:cutover-bundle` is expected FAIL with `local-evidence-invalid` until approved live token, owner-HMAC, customer ACL, and live Postgres inputs are rendered into fresh real prereq evidence and the generated-site preapply is rerun against that hash-bound site overlay.

## Previous Update - Launcher DOM Proof, Source Pinning, and Live Cutover Gate Hardening
