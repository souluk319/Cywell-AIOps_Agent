...
| `npm run verify:console:topology-dom:built` | PASS, 45 checks | Browser-required smoke with corpus reports, upload indexing, URL ingest, selected-document/full-corpus RAG, knowledge-menu query preservation, customer-change stale selected-document scope clearing, rendered RAG citations, Wiki Loop stages, selected-document note lineage, topology auto-load, empty/error reload stale-data protection, PBS-rich KPI/tone/signal-leader/link-filter rendering, PBS-rich signal-node RAG scope from selected context/upload lineage, Wiki Vault side-channel token/context/relation panels, filter-scoped node index, corpus/viewer deep links from PBS-rich topology nodes and context rows, orphan edge fallback nodes, mixed-scope graph selection, late-response stale-data protection, dense 28-node searchable index, and 1024px/390px overflow/overlap checks |
...
- Customer ID changes in the console Knowledge route clear selected corpus documents, viewer state, action results, topology state, and document query params before the next RAG/Wiki/topology request, preventing selected-document scope from crossing customer boundaries.
- Knowledge route navigation preserves the active `customer_id` and selected `document_id` query context when moving between customer data, RAG, LLM Wiki, and topology pages.
...
- The cutover bundle now requires generated-site live preapply evidence with current clean `fullHead`, exact overlay path, `--require-cluster`, `--require-secret`, and `--skip-applied`; it also requires exact real-render output hashes for PBS auth Secret, owner-HMAC Secret, Postgres Secret, live ConfigMap, customer ACL JSON, site kustomization, site customer ACL JSON, and redacted summary.
