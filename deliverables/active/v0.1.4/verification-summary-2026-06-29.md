...
| `npm run verify:console:topology-dom:built` | PASS, 47 checks | Browser-required smoke with corpus reports, upload indexing, URL ingest, selected-document/full-corpus RAG, knowledge-menu query preservation, customer-change stale selected-document scope clearing, late upload/topology response stale-data protection, browser back/forward route-state reconciliation, rendered RAG citations, Wiki Loop stages, selected-document note lineage, topology auto-load, empty/error reload stale-data protection, PBS-rich KPI/tone/signal-leader/link-filter rendering, PBS-rich signal-node RAG scope from selected context/upload lineage, Wiki Vault side-channel token/context/relation panels, filter-scoped node index, corpus/viewer deep links from PBS-rich topology nodes and context rows, orphan edge fallback nodes, mixed-scope graph selection, dense 28-node searchable index, and 1024px/390px overflow/overlap checks |
| `npm run verify:console:integration:built` | PASS, 105 checks | Includes static app `innerHTML` rejection, structural `/cywell/topology` manifest routing/navigation, topology auto-load, topology-selected source-scoped RAG request checks, Customer Corpus Workbench/viewer route checks, explicit all-corpus vs selected-document scope controls, URL ingest corpus joining for PBS `pages`/`upload_results`, selected-corpus RAG and selected-document wiki-loop/note request checks, Wiki-note document-lineage deep links, stale async scope guards, popstate URL reconciliation, PBS topology signal fields/counts, Signal leaders, Wiki Vault side-channel panels, and built plugin route/bundle checks |
...
| `npm run verify:pbs:cutover-bundle` | PASS, 9 self-test checks | Self-tests blocker extraction, artifact hashing, redaction, dirty/unpinned PBS source rejection, prereq self-test evidence rejection, missing prereq hash rejection, and live-prereq output directory binding |
...
- Knowledge route navigation preserves the active `customer_id` and selected `document_id` query context when moving between customer data, RAG, LLM Wiki, and topology pages; Wiki-note deep links now keep `document_id` lineage when a source document exists, and browser back/forward reconciles URL query state back into React state.
...
- The cutover bundle now requires generated-site live preapply evidence with current clean `fullHead`, exact overlay path, `--require-cluster`, `--require-secret`, and `--skip-applied`; it also requires exact real-render output hashes for PBS auth Secret, owner-HMAC Secret, Postgres Secret, live ConfigMap, customer ACL JSON, site kustomization, site customer ACL JSON, and redacted summary, all bound to the expected generated prereq output directories.
...
| `npm run render:pbs:cutover-bundle` | FAIL expected until evidence is regenerated | Current handoff bundle must be `PASS/live-preapply-ready` before live-ready claims. It remains invalid until the live prereq render is regenerated from the current clean Cywell HEAD and PBS source evidence is produced with strict `CAS_PBS_SOURCE_HEAD` pinning and a clean PBS checkout |
