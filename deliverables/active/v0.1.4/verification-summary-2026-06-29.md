...
| `npm run verify:deploy:manifests` | PASS, 260 checks | Includes base/shadow/live/CRC render checks, HMAC Secret refs, no tracked dev DB Secret, pbs-live Gateway customer ACL config, pbs-live Postgres release image pinning, PBS-compatible local schema checks, PBS-style local upload report/RAG source-scope checks, LLM Wiki staged contract checks, release image promotion force/evidence binding coverage, explicit strict shadow preflight script coverage, HTTPS PBS service-token transport, PBS live response scope guard coverage, structural Kubernetes API egress matching, PBS runtime endpoint readiness/Secret content/applied NetworkPolicy union self-checks, exact live lineage smoke self-checks, and direct console script build prerequisites |
...
- Strict live preflight checks the live customer ACL env wiring, rejects wildcard or prefix-wildcard customer ACL placeholders, confirms applied live ConfigMap values, requires release evidence sourced from current non-stale CRC deployment evidence, compares ready pod digests to promoted release evidence, and confirms all applied ingress/egress NetworkPolicies selecting knowledge-engine pods stay scoped.
...
- Customer ACL concrete validation now evaluates `default`, `owners`, `users`, and `groups`; pbs-live rejects wildcard or prefix/suffix wildcard entries anywhere and requires `default` to be empty before cutover.
...
The pbs-live overlay enforces Gateway customer workspace ACL through `cas-knowledge-live-config/customer-access-json`. The default rendered value is a wildcard placeholder; strict production preflight fails until it is replaced with concrete customer/group mapping.
