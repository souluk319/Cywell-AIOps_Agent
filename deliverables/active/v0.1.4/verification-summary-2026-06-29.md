...
| `npm run verify:console:integration` | PASS, 73 checks | Includes static app `innerHTML` rejection, structural `/cywell/topology` manifest routing/navigation, topology auto-load, topology-selected source-scoped RAG request checks, PBS topology signal fields/counts, Signal leaders, and built plugin route/bundle checks |
| `npm run verify:deploy:manifests` | PASS, 259 checks | Includes base/shadow/live/CRC render checks, HMAC Secret refs, no tracked dev DB Secret, pbs-live Gateway customer ACL config, pbs-live Postgres release image pinning, PBS-compatible local schema checks, PBS-style local upload report/RAG source-scope checks, release image promotion force/evidence binding coverage, explicit strict shadow preflight script coverage, HTTPS PBS service-token transport, PBS live response scope guard coverage, and direct console script build prerequisites |
