...
| `npm run verify:console:launcher-dom:built` | PASS, 9 checks | Browser-required smoke mounts the actual `useCASLauncher` context-provider hook through a fake SDK modal provider, verifies exactly one fixed bottom-right CAS launcher button, suppresses a pre-existing OpenShift Lightspeed launcher, prevents duplicates after rerender/remount, opens the panel, calls `/api/aiops/brainz`, posts read-only `/api/aiops/query` bodies, preserves conversation state, and reports no browser errors |
| `npm run verify:console:topology-dom:built` | PASS, 51 checks | Browser-required smoke with corpus reports, upload indexing, URL ingest, selected-document/full-corpus RAG, invalid `document_id` deep-link repair before RAG, invalid `document_id` clearing when reports are empty, knowledge-menu query preservation, customer-change stale selected-document scope clearing, late upload/RAG/topology response stale-data protection, Wiki-note deep links proving `document_id` is used by LLM Wiki requests, browser back/forward route-state reconciliation, rendered RAG citations, Wiki Loop stages, selected-document note lineage, topology auto-load, empty/error reload stale-data protection, PBS-rich KPI/tone/signal-leader/link-filter rendering, PBS-rich signal-node RAG scope from selected context/upload lineage, Wiki Vault side-channel token/context/relation panels, filter-scoped node index, corpus/viewer deep links from PBS-rich topology nodes and context rows, orphan edge fallback nodes, mixed-scope graph selection, dense 28-node searchable index, and 1024px/390px overflow/overlap checks |
| `npm run verify:console:integration:built` | PASS, 112 checks | Includes static app `innerHTML` rejection, structural `/cywell/topology` manifest routing/navigation, launcher native-Lightspeed suppression, topology auto-load, topology-selected source-scoped RAG request checks, Customer Corpus Workbench/viewer route checks, explicit all-corpus vs selected-document scope controls, URL ingest corpus joining for PBS `pages`/`upload_results`, selected-corpus RAG and selected-document wiki-loop/note request checks, Wiki-note document-lineage deep links, invalid query-document repair/empty-report clear guards, upload-report repair without clobbering topology/wiki viewer lineage, stale async customer/document scope guards, popstate URL reconciliation, PBS topology signal fields/counts, Signal leaders, Wiki Vault side-channel panels, and built plugin route/bundle checks |
| `npm run verify:pbs:source-contract:required` | PASS, 32 total checks: 31 PASS / 1 WARN, dirty-source WARN recorded | Verifies the real `F:\AI_Projects\PBS-Dev3` Docker/compose runtime contract, approved remote `git@github.com:souluk319/PBS_DEV_Part3.git`, port `8765`, `/api/health`, upload, URL ingest, chat/RAG, Wiki Vault, Wiki Loop, owner-scoped reports, selected uploads, graph, and chunk-preview API surface against the Cywell PBS runtime contract. Required evidence is written separately from optional evidence; strict release source pinning writes `test-results/cas-pbs-source-contract-pinned.json` |
| `npm run verify:pbs:live-prereqs` | PASS, 12 checks | Self-tests live prerequisite rendering, generated `pbs-live-site` overlay rendering with `oc`/`kubectl` fallback, redacted summary output, owner-HMAC Secret validation, wildcard/default/string wildcard/string-grant/broad group/non-string customer ACL rejection, Postgres database URL/Secret mismatch rejection, supported env-var error naming, and non-secret input template generation |
| `npm run verify:pbs:cutover-bundle` | PASS, 18 self-test checks | Self-tests blocker extraction, artifact hashing, redaction, live-prereq hash drift/path escape rejection, generated-site preapply hash binding/freshness, live runtime source revision binding, cluster-identity mismatch rejection, dirty local evidence rejection, dirty/unpinned PBS source rejection, exact PBS source contract hash-set validation, prereq self-test evidence rejection, missing prereq hash rejection, live-prereq output directory binding, and external blocker retention in active blockers when local evidence is invalid |
| `npm run verify:deploy:manifests` | PASS, 286 checks | Includes base/shadow/live/CRC render checks, HMAC Secret refs, no tracked dev DB Secret, generated pbs-live site preflight scripts, strict Gateway customer ACL checks, pbs-live Gateway customer ACL config, pbs-live Postgres release image pinning, PBS source-contract static coverage with split optional/required/pinned evidence and approved remote identity, private source-lane policy coverage, PBS-compatible local schema checks, PBS-style local upload report/RAG source-scope checks, LLM Wiki staged contract checks, release image promotion force/evidence binding coverage, explicit strict shadow preflight script coverage, HTTPS PBS service-token transport, PBS live response scope guard coverage, structural Kubernetes API egress matching, PBS runtime service-port/source-revision/endpoint health readiness checks, Secret content/applied NetworkPolicy union self-checks, CRC/release source and cluster identity binding, exact live lineage smoke self-checks, owner-HMAC live prereq/preflight checks, cutover bundle strict clean-source/real-render evidence checks, launcher DOM harness coverage, and direct console script build prerequisites |
...
Local JSON evidence is written under ignored `test-results/`. Some diagnostic artifacts are intentionally stale or non-live-ready; `test-results/cas-pbs-cutover-bundle.json` is the current handoff gate that rejects stale, dirty, cross-cluster, unapproved-remote, or unpinned evidence before live-ready claims. Strict pinned source evidence is required before live cutover and is generated only by `verify:release:source-pinning`.
...
test-results/cas-pbs-cutover-bundle.json
```
...
| `npm run verify:pbs:preflight:live:site:preapply` | FAIL expected, 47 PASS / 7 FAIL | Generated site overlay render/config/egress/API/release-image checks pass, including parsed rendered NetworkPolicy shape checks; service-token transport is HTTPS, pbs-live Gateway customer ACL is wired to the generated live ConfigMap replacement, owner-HMAC Secret validation passes from rendered prereq evidence, release evidence cluster identity matches the current cluster, `v0.1.4` release ImageStreamTags resolve to promoted digests, and pbs-live Postgres image pinning passes. Current CRC still has no `playbookstudio` namespace/service, no `cas-pbs-auth`, no `cas-knowledge-postgres-live`, and still has the legacy `cas-knowledge-postgres` dev Secret |
| `npm run render:pbs:cutover-bundle` | FAIL expected until evidence is regenerated | Current handoff bundle must be `PASS/live-preapply-ready` before live-ready claims. It remains invalid until this tree is committed/clean, real live prereq render evidence is regenerated from the current clean HEAD, generated-site preapply evidence is fresh and hash-bound to that render, `test-results/cas-pbs-source-contract-pinned.json` is produced by strict full-SHA `CAS_PBS_SOURCE_HEAD` pinning with an approved clean PBS checkout, and live preapply records PBS runtime pods stamped with that same SHA. External live blockers are reported both as external preapply blockers and active blockers |
