...
| `npm run verify:console:topology-dom:built` | PASS, 49 checks | Browser-required smoke with corpus reports, upload indexing, URL ingest, selected-document/full-corpus RAG, knowledge-menu query preservation, customer-change stale selected-document scope clearing, late upload/RAG/topology response stale-data protection, Wiki-note deep links proving `document_id` is used by LLM Wiki requests, browser back/forward route-state reconciliation, rendered RAG citations, Wiki Loop stages, selected-document note lineage, topology auto-load, empty/error reload stale-data protection, PBS-rich KPI/tone/signal-leader/link-filter rendering, PBS-rich signal-node RAG scope from selected context/upload lineage, Wiki Vault side-channel token/context/relation panels, filter-scoped node index, corpus/viewer deep links from PBS-rich topology nodes and context rows, orphan edge fallback nodes, mixed-scope graph selection, dense 28-node searchable index, and 1024px/390px overflow/overlap checks |
| `npm run verify:console:integration:built` | PASS, 109 checks | Includes static app `innerHTML` rejection, structural `/cywell/topology` manifest routing/navigation, topology auto-load, topology-selected source-scoped RAG request checks, Customer Corpus Workbench/viewer route checks, explicit all-corpus vs selected-document scope controls, URL ingest corpus joining for PBS `pages`/`upload_results`, selected-corpus RAG and selected-document wiki-loop/note request checks, Wiki-note document-lineage deep links, document-query fallback scope, upload-report repair without clobbering topology/wiki viewer lineage, stale async customer/document scope guards, popstate URL reconciliation, PBS topology signal fields/counts, Signal leaders, Wiki Vault side-channel panels, and built plugin route/bundle checks |
| `npm run verify:pbs:source-contract:required` | PASS, 31 total checks: 30 PASS / 1 WARN, dirty-source WARN recorded | Verifies the real `F:\AI_Projects\PBS-Dev3` Docker/compose runtime contract, port `8765`, `/api/health`, upload, URL ingest, chat/RAG, Wiki Vault, Wiki Loop, owner-scoped reports, selected uploads, graph, and chunk-preview API surface against the Cywell PBS runtime contract. Evidence records PBS source branch, HEAD, tree status, optional expected HEAD, optional clean-tree enforcement flag, and contract file hashes |
| `npm run verify:pbs:live-prereqs` | PASS, 11 checks | Self-tests live prerequisite rendering, generated `pbs-live-site` overlay rendering with `oc`/`kubectl` fallback, redacted summary output, owner-HMAC Secret validation, wildcard/default/string wildcard/broad group/non-string customer ACL rejection, Postgres database URL/Secret mismatch rejection, supported env-var error naming, and non-secret input template generation |
| `npm run verify:pbs:cutover-bundle` | PASS, 14 self-test checks | Self-tests blocker extraction, artifact hashing, redaction, live-prereq hash drift/path escape rejection, generated-site preapply hash binding, cluster-identity mismatch rejection, dirty/unpinned PBS source rejection, prereq self-test evidence rejection, missing prereq hash rejection, live-prereq output directory binding, and external blocker retention when local evidence is invalid |
...
- Customer ACL concrete validation now evaluates `owners`, `users`, and `groups`; pbs-live rejects `default`, wildcard or prefix/suffix wildcard entries, string ACL values, non-string customer entries, placeholder principals/customers, and broad Kubernetes system groups before cutover.
...
- Knowledge route navigation preserves the active `customer_id` and selected `document_id` query context when moving between customer data, RAG, LLM Wiki, and topology pages; Wiki-note deep links now keep `document_id` lineage when a source document exists, deep-linked notes use that document scope before upload reports load, and browser back/forward reconciles URL query state back into React state.
...
- The cutover bundle now requires generated-site live preapply evidence with current clean `fullHead`, exact overlay path, `--require-cluster`, `--require-secret`, and `--skip-applied`; it also requires cluster identity to match CRC deployment/release evidence, recomputes the current generated live-prereq file/site-overlay/redacted-summary hashes, requires strict full-SHA PBS source pinning, and reports external live blockers even when local evidence is invalid.
...
Local JSON evidence was written under ignored `test-results/`. Some diagnostic artifacts are intentionally stale or non-live-ready; `test-results/cas-pbs-cutover-bundle.json` is the current handoff gate that rejects stale, dirty, cross-cluster, or unpinned evidence before live-ready claims.
...
| `npm run render:pbs:cutover-bundle` | FAIL expected until evidence is regenerated | Current handoff bundle must be `PASS/live-preapply-ready` before live-ready claims. It remains invalid while the git tree is dirty, live prereq render evidence is stale/dirty or hash-mismatched, generated-site preapply evidence is not from a current clean tree, and PBS source evidence is not produced by strict full-SHA `CAS_PBS_SOURCE_HEAD` pinning with a clean PBS checkout. External live blockers are still reported separately in the bundle |
