...
- `verify:pbs:source-contract:required` now verifies the clean v0.1.4 PBS source clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone` by default, falling back to `F:\AI_Projects\PBS-Dev3` only when the companion clone is absent. It checks the Docker/compose runtime contract, approved remote identity, port `8765`, `/api/health`, upload, URL ingest, chat/RAG, public chat gateway controls, Wiki Vault, Wiki Loop, owner-scoped upload reports, selected uploads, graph, and chunk-preview API surface before v0.1.4 claims PBS source compatibility. `verify:release:source-pinning` is the stricter release gate: it writes `test-results/cas-pbs-source-contract-pinned.json` only when `CAS_PBS_SOURCE_HEAD` is a full approved SHA, the PBS checkout is clean, the PBS remote matches the approved GitHub host/repository, `git fetch --prune origin` succeeds, at least one fetched `origin/*` branch contains the pinned SHA, and the expected source contract file hashes are present. The cutover bundle also rejects stale pinned source proof, so remote containment must be proven again close to live handoff.
- Current v0.1.4 strict source pin evidence passes against clean PBS clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone`, branch `kugnus/cywell-v0.1.4-source-contract`, SHA `6604777abb9e6bd44a83c6a12f36e31ac396489e`, on approved remote `https://github.com/souluk319/PBS_DEV_Part3.git`. The same branch now includes `deploy/openshift-cywell-v014` so the real PBS app can be deployed under Cywell's expected `playbookstudio` runtime Service contract.
...
- `deploy:pbs:runtime:crc` now performs a full CRC rehearsal of the approved PBS runtime overlay in namespace `playbookstudio`: source-bound local PBS runtime image build/reuse, ordered DB migration and seed replay from that image digest, pinned source overlay, graph foundation migration, explicit CRC wiki-loop compatibility hotfix stamp, HTTPS Service readiness, ready endpoint check, source revision check, ready runtime pod image digest binding, and `/api/health` readiness for DB, pgvector, zero missing/stale embedding entries, corpus/index counts, required scopes, and compiled wiki.
...
- 원본 `F:\AI_Projects\PBS-Dev3` 워킹트리는 여전히 더럽기 때문에 production/live evidence로 쓰지 않는다. v0.1.4 소스 기준점은 clean clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone`의 branch `kugnus/cywell-v0.1.4-source-contract`, SHA `6604777abb9e6bd44a83c6a12f36e31ac396489e`로 고정했고 `verify:release:source-pinning`이 PASS했다. Live preflight/cutover evidence still requires ready PBS runtime pods to expose that same source revision and report `/api/health` runtime/corpus readiness through the Service DNS.
...
- source/API contract verification against clean clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone` through `verify:pbs:source-contract:required`, including PBS source branch/HEAD/tree status, approved remote identity, public chat gateway contract, and contract file hashes; current strict `verify:release:source-pinning` evidence passes for PBS SHA `6604777abb9e6bd44a83c6a12f36e31ac396489e`; live cutover additionally requires freshness-valid pinned evidence in `test-results/cas-pbs-source-contract-pinned.json`, proof from a successful fresh fetch that an approved `origin/*` ref contains the pinned PBS SHA, release evidence proving the Cywell HEAD is also present in an approved fetched `origin/*` ref, expected live cluster identity binding through `CAS_RELEASE_EXPECTED_CLUSTER_IDENTITY_JSON`, PBS runtime pod source stamps matching the same approved SHA, and Service-DNS `/api/health` readiness for DB, pgvector, corpus/index, compiled wiki, and required scopes
