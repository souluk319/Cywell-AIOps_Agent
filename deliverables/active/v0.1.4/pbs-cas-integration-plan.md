...
- Current v0.1.4 strict source pin evidence passes against clean PBS clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone`, branch `kugnus/cywell-v0.1.4-source-contract`, SHA `7b3c28fccc656cf862065f0e6877e4dd3a71e0f1`, on approved remote `https://github.com/souluk319/PBS_DEV_Part3.git`. The same branch now includes `deploy/openshift-cywell-v014` so the real PBS app can be deployed under Cywell's expected `playbookstudio` runtime Service contract.
...
- 원본 `F:\AI_Projects\PBS-Dev3` 워킹트리는 여전히 더럽기 때문에 production/live evidence로 쓰지 않는다. v0.1.4 소스 기준점은 clean clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone`의 branch `kugnus/cywell-v0.1.4-source-contract`, SHA `7b3c28fccc656cf862065f0e6877e4dd3a71e0f1`로 고정했고 `verify:release:source-pinning`이 PASS했다. Live preflight/cutover evidence still requires ready PBS runtime pods to expose that same source revision and report `/api/health` runtime/corpus readiness through the Service DNS.
