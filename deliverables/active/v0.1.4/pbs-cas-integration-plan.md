...
- source/API contract verification against `F:\AI_Projects\PBS-Dev3` through `verify:pbs:source-contract:required`, including PBS source branch/HEAD/tree status, approved remote identity, and contract file hashes; live cutover additionally requires strict `verify:release:source-pinning` evidence in `test-results/cas-pbs-source-contract-pinned.json`, proof that a fetched approved `origin/*` ref contains the pinned PBS SHA, PBS runtime pod source stamps matching the same approved SHA, and Service-DNS `/api/health` readiness for DB, pgvector, corpus/index, compiled wiki, and required scopes
