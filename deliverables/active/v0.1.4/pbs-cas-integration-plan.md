...
- 원본 `F:\AI_Projects\PBS-Dev3` 워킹트리는 여전히 더럽기 때문에 production/live evidence로 쓰지 않는다. v0.1.4 소스 기준점은 clean clone `F:\AI_Projects\PBS-Dev3-cywell-v014-source-pin-clone`의 branch `kugnus/cywell-v0.1.4-source-contract`, SHA `0c6b23c46acb6d3a7d87f416b6512d23f53a8c3b`로 고정했고 `verify:release:source-pinning`이 PASS했다. Live preflight/cutover evidence still requires ready PBS runtime pods to expose that same source revision and report `/api/health` runtime/corpus readiness through the Service DNS.
