...
Gateway knowledge private routes verify the incoming OpenShift Console `UserToken` through Kubernetes `SelfSubjectReview`, strip user bearer tokens before the internal Knowledge Engine hop, and inject only the Gateway-derived owner header. Public knowledge health is sanitized so storage paths, tenant counts, and PBS internals are not exposed through the Console proxy.
...
`deploy:crc`는 `.env`, `deliverables`, `node_modules`를 제외한 최소 build context를 `test-results/cas-build-context`에 만든 뒤 OpenShift binary build로 `cas-gateway:dev`, `cas-console-plugin:dev`, `cas-knowledge-engine:dev` 이미지를 생성합니다. 깨끗한 CRC 클러스터에서도 namespace와 ImageStream을 먼저 만들고, 기존 v0.1.3 operator가 없으면 optional pause를 건너뛴 뒤 manifest 적용, rollout 대기, ConsolePlugin 활성화, deployment 검증을 수행합니다.
