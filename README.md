...
The v0.1.4 base deployment uses the local/Postgres PBS-compatible Knowledge Engine. PBS live cutover remains gated on external `playbookstudio` runtime, `cas-pbs-auth`, live Postgres Secret, live overlay application, and PBS runtime/corpus readiness. Local `v0.1.4` release ImageStreamTags can be prepared from the verified CRC images with `npm run release:crc:v0.1.4`.
